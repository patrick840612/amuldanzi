...
public interface MapRepository extends CrudRepository<MemberPetDTO, String>{
	...
	@Query(value = "SELECT mi.sido sido, mi.blood_tel blood_tel, mp.which_pet whichPet, mp.pet_blood petBlood \r\n"
	        + "FROM member_info mi \r\n"
	        + "JOIN member_pet mp ON mi.id = mp.id WHERE LEFT(mi.sido, 2) = LEFT(:sido, 2)", nativeQuery = true)
