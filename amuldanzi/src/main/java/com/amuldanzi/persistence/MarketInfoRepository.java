...
public interface MarketInfoRepository extends CrudRepository<MarketInfoDTO, Integer>{
	...
	@Query(value = "SELECT m.title AS title, m.addr AS addr, m.tel AS tel, m.time AS time, m.lat AS lat, m.lng AS lng FROM market m WHERE m.addr LIKE %:addr%", nativeQuery = true)
