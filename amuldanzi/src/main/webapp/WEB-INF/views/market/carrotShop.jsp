<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<!DOCTYPE html>
<html class="light-theme">
<head>
<meta charSet="utf-8" />
<title>애물단지 마켓</title>




<link rel="preload"
	href="/chunks/css/market/marketShop.css"
	as="style" />
<link rel="stylesheet"
	href="/chunks/css/market/marketShop.css"/>
<link rel="preload"
	href="/chunks/css/market/marketShop2.css"
	as="style" />
<link rel="stylesheet"
	href="/chunks/css/market/marketShop2.css"/>


<script
	src="https://contentweaver-static.kr.karrotmarket.com/_next/static/chunks/webpack-a98067593edd9ace.js"
	defer=""></script>

	
	<jsp:include page="../main/header.jsp"></jsp:include>
</head>

<body>
	<div id="__next">
		<div class="ye5i0m0">
			<div class="_10rqyav0">
				
				<!-- 여기까지 -->
				<div class="_10rqyav1">
					<div class="_1qbix3d0">
						<section class="_3nqg3y0">
							<div class="_3nqg3y1 _1ff3f302">
								<h1 class="_3nqg3y2">
									애물단지에 등록된
									<!-- -->
									<br />믿을 수 있는 가게들!
									<!-- -->
								</h1>
								<span class="_3nqg3y3">다양한 가게들을 <!-- -->
									<br />한눈에 보고 찾아보세요.<!-- --></span>
								<div class="_3nqg3y4">
									<span
										style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0; margin: 0; padding: 0; position: absolute; top: 0; left: 0; bottom: 0; right: 0"><img
										alt="배너 이미지"
										src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
										decoding="async" data-nimg="fill" class="_19qbbiq0 undefined"
										style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-sizing: border-box; padding: 0; border: none; margin: auto; display: block; width: 0; height: 0; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover" />
									<noscript>
											<img alt="배너 이미지"
												src="/_next/static/media/nearby_stores_hero_image_3x.47c43e04.png"
												decoding="async" data-nimg="fill"
												style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-sizing: border-box; padding: 0; border: none; margin: auto; display: block; width: 0; height: 0; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover"
												class="_19qbbiq0 undefined" loading="lazy" />
										</noscript></span>
								</div>
							</div>
						</section>
						<div class="_1qbix3d1 _1ff3f300 _1ff3f302">
							<section class="_1m0guz80">
								<h2 class="_1m0guz81">애물단지 마켓</h2>
								<div class="vb66m70">
									<ul class="vb66m71 korcsc5" role="list">
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg41 _1gdwmg44"
											role="radio" tabindex="0">전체<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="6375d32a3029e437ecd0a8fb" /></label></li>
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg42 _1gdwmg44"
											role="radio" tabindex="0">카테고리1<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="63747eec1efd3441dec8ca71" /></label></li>
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg42 _1gdwmg44"
											role="radio" tabindex="0">카테고리2<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="63747faf1efd3441dec8ca72" /></label></li>
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg42 _1gdwmg44"
											role="radio" tabindex="0">카테고리3<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="6375d2903029e437ecd0a8f8" /></label></li>
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg42 _1gdwmg44"
											role="radio" tabindex="0">카테고리4<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="6375d2c03029e437ecd0a8f9" /></label></li>
										<li class="vb66m72 korcsc5" role="listitem"><label
											class="chip-base _1gdwmg40 korcsc3 korcsc4 _1gdwmg42 _1gdwmg44"
											role="radio" tabindex="0">카테고리5<!-- -->
												<input name="store-filter" type="radio" tabindex="-1"
												class="chip-input _1gdwmg46"
												value="6375d2f63029e437ecd0a8fa" /></label></li>
									</ul>
								</div>
								<!--$-->
								<div class="_4a1tzy0">
									<ul class="_4a1tzy1 korcsc5">
										<li class="_4a1tzy2"><a
											class="smb-list-item _1o1a6ke0 korcsc4"
											href="/kr/business-profiles/%EC%8B%9C%EA%B3%A8%EC%B2%AD%EB%85%84-d2f00db4a9cc4fb986a85037b8378955/">
											
												<div class="smb-list-item-thumbnail-wrapper _1o1a6ke2 _1o1a6ke1">
													<div class="thumbnail-base qudd1g0 qudd1g4 _1o1a6ke1">
														<img
															src=""
															class="thumbnail-image qudd1g9" />
													</div>
												</div>
												
												<div class="smb-list-item-content _1o1a6ke3">
													<div class="smb-list-item-name-wrapper _1o1a6ke4">
														<span class="smb-list-item-name _1o1a6ke5">가게이름</span>
														<span class="smb-list-item-region _1o1a6ke6">주소</span>
													</div>
													<span class="smb-list-item-description _1o1a6ke7">내용</span><span
														class="smb-list-item-infos _1o1a6ke8">후기 0 ∙ 카테1 ∙ 카테2</span>
												</div></a></li>
										
									</ul>
								</div>
								<!--/$-->
							</section>
							
							<div class="_1irmkvy0">
								<section class="_1irmkvy1">
									<div class="_1irmkvy2">
										<span
											style="box-sizing: border-box; display: block; overflow: hidden; width: initial; height: initial; background: none; opacity: 1; border: 0; margin: 0; padding: 0; position: absolute; top: 0; left: 0; bottom: 0; right: 0"><img
											alt="town image"
											src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
											decoding="async" data-nimg="fill" class="_19qbbiq0 undefined"
											style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-sizing: border-box; padding: 0; border: none; margin: auto; display: block; width: 0; height: 0; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover" />
										<noscript>
												<img alt="town image"
													src="/_next/static/media/ic_category_town_colored_m.13979d69.svg"
													decoding="async" data-nimg="fill"
													style="position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-sizing: border-box; padding: 0; border: none; margin: auto; display: block; width: 0; height: 0; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover"
													class="_19qbbiq0 undefined" loading="lazy" />
											</noscript></span>
									</div>
									<div class="_1irmkvy3">
										<h6 class="_1irmkvy4">가게를 운영하고 계신가요?</h6>
										<span class="_1irmkvy5 _1irmkvy4">애물단지 사업자등록을 하고 마켓을 이용해보세요!</span>
									</div>
									<div class="_1irmkvy6">
										<a as="a" href="https://business.daangn.com" target="_blank"
											class="karrot-button _1aul9fg0 korcsc3 korcsc4 _1aul9fg1 _1aul9fg8 _1aul9fgj _1irmkvy7">사업자등록 하러가기</a>
									</div>
								</section>
							</div>
						</div>
					</div>
				</div>
				
			</div>
		</div>
		<div id="qr-modal-portal"></div>
		<div id="toast-portal"></div>
	</div>
	<noscript>
		<iframe src="https://www.googletagmanager.com/ns.html?id=GTM-WRVTQ3H"
			width="0" height="0" style="display: none; visibility: hidden"></iframe>
	</noscript>
	</body>
</html>